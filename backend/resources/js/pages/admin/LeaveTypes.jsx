...
const blank = { name:'', code:'', quota_per_year:12, is_paid:true, requires_attachment:false, is_carry_over_allowed:false, max_carry_over_days:0, is_active:true };
...
export default function LeaveTypes(){
    ...
    return <div className="mx-auto max-w-6xl space-y-5 p-6 md:p-8">
        <div className="flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-bold text-slate-900">Jenis Cuti & Kuota</h1><p className="text-slate-500">Tentukan jumlah hari yang diberikan kepada setiap karyawan per tahun.</p></div><button onClick={()=>{setEditing(null);setForm(blank);setOpen(true)}} className="flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 font-semibold text-white"><Plus size={18}/>Tambah Jenis Cuti</button></div>
        ...
        {open&&<form onSubmit={save} className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 md:grid-cols-2">
            ...
            <label className="text-sm font-semibold text-slate-700">Kuota yang diberikan per tahun<input type="number" min="0" max="366" required value={form.quota_per_year} onChange={e=>setForm({...form,quota_per_year:Number(e.target.value)})} className="mt-2 w-full rounded-xl border p-3"/><small className="mt-1 block font-normal text-slate-500">Karyawan tidak dapat mengajukan melebihi sisa jumlah hari ini.</small></label>
            ...
            <div className="flex flex-wrap gap-5 md:col-span-2">{[['is_paid','Cuti dibayar'],['requires_attachment','Wajib lampiran'],['is_carry_over_allowed','Bisa dibawa ke tahun berikutnya'],['is_active','Aktif']].map(([k,l])=><label key={k} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={Boolean(form[k])} onChange={e=>setForm({...form,[k]:e.target.checked})}/> {l}</label>)}</div>
