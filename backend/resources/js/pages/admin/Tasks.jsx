...
export default function AdminTasks() {
    ...
    const load = async () => {
        ...
        try {
            const [taskResponse, employeeResponse] = await Promise.all([api.get('/admin/tasks'), api.get('/admin/employees')]);
    ...
    return <div className="mx-auto max-w-7xl space-y-6 p-5 md:p-8">
        ...
        {loading?<div className="flex justify-center py-20"><Loader2 className="animate-spin text-emerald-700"/></div>:<div className="overflow-x-auto rounded-2xl border bg-white"><table className="w-full text-sm"><thead className="bg-slate-50"><tr><th className="p-4 text-left">Karyawan</th><th className="p-4 text-left">Tugas</th><th className="p-4 text-left">Jenis/Jadwal</th><th className="p-4 text-left">Status</th><th className="p-4 text-right">Aksi</th></tr></thead><tbody>{items.map(item=><tr key={item.id} className="border-t"><td className="p-4"><strong>{item.employee?.full_name}</strong><span className="block text-xs text-slate-500">{item.employee?.employee_number}</span></td><td className="p-4"><strong>{item.title}</strong><span className="block max-w-sm text-xs text-slate-500">{item.description||'—'}</span></td><td className="p-4">{item.is_habit?`Habit • ${item.recurrence_rule}`:`Daily • ${item.task_date?.slice(0,10)}`}</td><td className="p-4">{item.is_active?'Aktif':'Selesai/nonaktif'}</td><td className="p-4 text-right"><button onClick={()=>startEdit(item)} className="mr-2 rounded-lg bg-blue-50 p-2 text-blue-700"><Pencil className="h-4 w-4"/></button><button onClick={()=>remove(item)} className="rounded-lg bg-rose-50 p-2 text-rose-700"><Trash2 className="h-4 w-4"/></button></td></tr>)}</tbody></table>{!items.length&&<div className="p-12 text-center text-slate-500">Belum ada tugas.</div>}</div>}
    </div>;
}
