...
|---|---|---|
| 0 — Foundation | Project setup, design system, auth, multi-tenant data model, CI/CD | 2–3 weeks |
| 1 — Attendance MVP | Device binding, face enrollment/verification, anti-fake GPS check-in, basic attendance history, push notifications | 4–6 weeks |
| 2 — Workforce management | Shift scheduling, leave + multilevel approval, company calendar, urgent broadcast | 4–5 weeks |
| 3 — Payroll & finance | Payroll engine, BPJS/PPh 21 engine, salary slip PDFs, claims/reimbursement | 5–6 weeks |
| 4 — Dashboard & personal scheduler | Realtime admin dashboard widgets, personal habit scheduler, exports/reporting | 3–4 weeks |
| 5 — Hardening & launch | Security audit/pentest, low-end device performance pass, UAT, store submission | 2–3 weeks |

Total: roughly **5–6 months** for a small dedicated team. If you're building this solo or near-solo, treat Phase 1 as a shippable MVP on its own before taking on payroll's compliance complexity — attendance alone is already a complete, sellable product.
...
33 screens across 8 reusable UI patterns. Once a pattern is designed, every screen that shares it is a data/copy swap, not a new design — that's the leverage point, and it's why the patterns below matter more than the raw screen count.

| # | Screen | Pattern | Status |
|---|---|---|---|
| **Onboarding & auth** | | | |
| 1 | Splash | Brand moment | Designed |
| 2 | Onboarding carousel (3 slides) | Carousel | Designed (1 of 3 slides shown) |
| 3 | Login / company workspace entry | Form entry | Designed |
| 4 | Device-binding confirmation | Form entry | Inventoried |
| 5 | Face enrollment (3-step capture) | Guided capture | Designed |
| 6 | Forgot password / OTP | Form entry | Inventoried |
| **Home & attendance** | | | |
| 7 | Home dashboard | Hub | Designed |
| 8 | Face check-in | Guided capture | Designed |
| 9 | Check-in success state | Confirmation | Inventoried |
| 10 | Attendance history | List/history | Designed |
| 11 | Attendance detail (map + photo) | Detail/breakdown | Inventoried |
| 12 | Weekly/monthly report | Detail/breakdown | Inventoried |
| **Shift & personal schedule** | | | |
| 13 | Shift schedule | List/history | Inventoried |
| 14 | Personal habit list | List/history | Inventoried |
| 15 | Add/edit habit | Form entry | Inventoried |
| **Leave & approval** | | | |
| 16 | Leave request | Form entry | Designed |
| 17 | My leave requests | List/history | Inventoried |
| 18 | Leave detail | Detail/breakdown | Inventoried |
| 19 | Approval inbox | Action-list | Designed |
| 20 | Approval detail | Detail/breakdown | Inventoried |
| **Claims** | | | |
| 21 | Claim submission | Form entry | Inventoried |
| 22 | Claim list | List/history | Inventoried |
| 23 | Claim detail | Detail/breakdown | Inventoried |
| **Payroll** | | | |
| 24 | Payslip list | List/history | Inventoried |
| 25 | Payslip detail | Detail/breakdown | Designed |
| **Calendar & announcements** | | | |
| 26 | Company calendar | Calendar (library component) | Inventoried |
| 27 | Event detail | Detail/breakdown | Inventoried |
| 28 | Urgent announcements feed | List/history | Inventoried |
| 29 | Announcement detail | Detail/breakdown | Inventoried |
| **Notifications & profile** | | | |
| 30 | Notifications center | List/history | Inventoried |
| 31 | Profile | Settings list | Inventoried |
| 32 | Settings | Settings list | Inventoried |
| 33 | Help/support | Settings list | Inventoried |

"Inventoried" screens reuse a pattern already designed above — building screen 17 (my leave requests) means taking the exact layout from screen 10 (attendance history) and swapping the row content and badge labels, not designing from scratch.

**Where Teka appears:** splash (hero), onboarding (guide), face enrollment and check-in (reassurance during the two moments most likely to cause friction), and check-in success (celebration). Deliberately absent from list, form, approval, and payslip screens — a mascot on every screen stops registering and starts feeling like clutter. He shows up at emotional moments, not administrative ones.
...
```
Bottom nav (persistent, 4 tabs)
├── Home            → dashboard (hub)
├── Absensi         → attendance history → attendance detail
├── Approval        → approval inbox → approval detail
│                      (employees see an empty state here, not a hidden tab —
│                       hiding tabs by role adds inconsistency across devices/sessions)
└── Profil          → profile → settings / help

Modal / full-screen flows (not part of bottom nav)
├── Onboarding & auth      → splash → carousel → login → device binding → face enrollment
├── Face check-in          → launched from Home, returns to Home on success
├── Leave request          → launched from Home quick actions or Absensi tab
├── Claim submission       → launched from Home quick actions
└── Notifications          → launched from a bell icon in the top bar, not a bottom tab
```

Each bottom-nav tab owns its own stack (`go_router`'s `StatefulShellRoute` is the right fit here), so switching tabs preserves scroll position and doesn't reset in-progress navigation — standard expectation for this kind of app, easy to get wrong if navigation is bolted on late.

---

## 14. Next steps

Status as of this revision: PoC scope agreed (Supabase backend, Flutter mobile only, no web admin yet), and the full UI/UX pattern library above is designed.

1. Build the face check-in flow first — real camera, ML Kit, and GPS (including the mock-location check), writing to a minimal or mocked Supabase table. This is the screen that proves or disproves the riskiest technical assumption in the whole app.
2. Wire the onboarding → login → device-binding → enrollment flow second, since check-in depends on an enrolled face existing.
3. From there, extend outward using the patterns in section 12 — list screens and form screens are now fast to produce because the pattern is already set.
4. When the PoC is validated, revisit section 4 (backend) to decide whether Supabase carries through to production or whether the original Laravel + Filament recommendation takes over for the full build — worth an explicit decision rather than letting the PoC choice become permanent by default.
