...
class _FaceCheckInScreenState extends State<FaceCheckInScreen> {
  ...
  Future<void> _loadFaceReferences() async {
      ...
      if (cached != null) {
        try {
          ...
          references = raw.isNotEmpty && raw.first is List
              ? raw.map((item) => (item as List).map((value) => (value as num).toDouble()).toList()).toList()
    ...
    setState(() {
      ...
      _faceReferenceError = _registeredEmbeddings.isEmpty
          ...
          : null;
      _liveSimilarity = _calculateBestSimilarity(_liveEmbedding, references ?? const []);
    });
  ...
  Future<Map<String, dynamic>> _loadTodayInfo() async {
    ...
    for (final shift in shifts) {
      ...
      final canUse = widget.isCheckOut
          ? attendance != null && attendance['check_in_time'] != null && attendance['check_out_time'] == null
          : attendance == null;
      if (canUse) { selected = shift; break; }
    }
    if (mounted) setState(() {
  ...
  void _handleFaceEmbedding(List<double> embedding) {
    ...
    final bestScore = _calculateBestSimilarity(embedding, _registeredEmbeddings);
  ...
  double _calculateBestSimilarity(
    ...
    for (final reference in references) {
      final score = FaceMatcherService.calculateCosineSimilarity(liveEmbedding, reference);
  ...
  void _scheduleAutomaticCapture() {
    if (!_isReadyForAutomaticCapture || (_autoCaptureTimer?.isActive ?? false)) return;
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              widget.isCheckOut ? 'Absen Keluar' : 'Absen Masuk',
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Center(
                                    child: Stack(
                                      ...
                                      children: [
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            border: Border.all(
                                              color: _liveSimilarity >= .80
                                                  ? AppColors.primary
                                                  : (_isTooDark ? AppColors.error : Colors.grey[300]!),
                                              width: 4.w,
                                            ),
                                          ),
                                          child: ClipRRect(
                                        ...
                                        ).animate(target: (_isFaceProper && !_isTooDark) ? 1 : 0).scale(duration: 300.ms, curve: Curves.easeOutBack, end: const Offset(1.015, 1.015)),
                                          ...
                                          Positioned.fill(
                                            child: ClipRRect(
                                              ...
                                              child: Align(
                                                ...
                                                ).animate(onPlay: (c) => c.repeat(reverse: true)).moveY(begin: 8.h, end: 340.h, duration: 1500.ms, curve: Curves.easeInOut),
                                              ),
                                            ),
                                          ),
                                      ],
                                  ...
                                  Text(
                                    _isTooDark
                                        ...
                                        : _liveSimilarity >= .80
                                            ...
                                            : _isFaceProper && _registeredEmbeddings.isNotEmpty
                                    ...
                                    builder: (context, snapshot) {
                                      ...
                                      if (snapshot.hasData && snapshot.data != null) {
                                        ...
                                        final shifts = (data['shifts'] as List? ?? const [])
                                            .map((item) => Map<String, dynamic>.from(item as Map))
                                            .toList();
                                        final availableShifts = shifts.where((shift) {
                                          final attendance = shift['attendance'] as Map?;
                                          return widget.isCheckOut
                                              ? attendance != null && attendance['check_in_time'] != null && attendance['check_out_time'] == null
                                              : attendance == null;
                                        }).toList();
                                        final role = Map<String, dynamic>.from(data['role'] as Map? ?? {});
                                        return Container(
                                            ...
                                            children: [
                                              Row(
                                                children: [
                                                  ...
                                                  SizedBox(width: 8.w),
                                                  Expanded(child: Text(role['employee_name']?.toString() ?? 'Karyawan', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13.sp, color: Colors.blue[800]))),
                                                ],
                                              ),
                                              SizedBox(height: 5.h),
                                              Text('${role['department'] ?? 'Belum ada departemen'} • ${role['position'] ?? 'Belum ada jabatan'}', style: TextStyle(fontSize: 12.sp, color: Colors.blue[800])),
                                              SizedBox(height: 12.h),
                                              if (availableShifts.isEmpty)
                                                Container(
                                                  padding: EdgeInsets.all(10.w),
                                                  decoration: BoxDecoration(color: AppColors.warningAmber.withValues(alpha: .12), borderRadius: BorderRadius.circular(10.r)),
                                                ...
                                                DropdownButtonFormField<String>(
                                                  initialValue: _selectedShiftKey,
                                                  isExpanded: true,
                                                  decoration: const InputDecoration(labelText: 'Shift untuk absensi ini', prefixIcon: Icon(Icons.schedule)),
                                                  items: availableShifts.map((shift) => DropdownMenuItem(
                                                    value: _shiftKey(shift),
                                                    child: Text('${shift['name']} • ${shift['start_time']}–${shift['end_time']}'),
                                                  )).toList(),
                                                  onChanged: (key) {
                                                    setState(() {
                                                      _selectedShiftKey = key;
                                                      _selectedShift = availableShifts.firstWhere((shift) => _shiftKey(shift) == key);
                                                    });
                                                    _scheduleAutomaticCapture();
                                                  },
                                                ),
                                              ...
                                              Row(
                                                children: [
                                                  Icon(widget.isCheckOut ? Icons.logout : Icons.login, color: Colors.grey[700], size: 16.w),
                                                  SizedBox(width: 6.w),
                                                  Expanded(child: Text(widget.isCheckOut
                                                      ? 'Jadwal pulang ${_selectedShift!['end_time']}'
                                                      : 'Jadwal masuk ${_selectedShift!['start_time']} • pulang ${_selectedShift!['end_time']}', style: TextStyle(fontSize: 12.sp, color: Colors.grey[800]))),
                                                  Text(_selectedShift!['time_status_label']?.toString() ?? '', style: TextStyle(fontSize: 10.sp, fontWeight: FontWeight.w700, color: AppColors.primary)),
                                                ],
                                              ),
                                              ],
                                            ],
                                      ...
                                      return Container(
                                        ...
                                        decoration: BoxDecoration(color: AppColors.error.withValues(alpha: .08), borderRadius: BorderRadius.circular(12.r)),
                                        child: const Text('Informasi shift gagal dimuat. Periksa koneksi lalu buka ulang halaman.'),
                                      );
                                    }
                                  ),
                                  ...
                                  AnimatedContainer(
                                    ...
                                    decoration: BoxDecoration(
                                      color: isLoading ? AppColors.primary : AppColors.primary.withValues(alpha: .10),
                                      borderRadius: BorderRadius.circular(16.r),
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        if (isLoading)
                                          ...
                                          Icon(Icons.center_focus_strong, color: AppColors.primary, size: 22.w),
                                        SizedBox(width: 10.w),
                                        Flexible(
                                          child: Text(
                                            isLoading
