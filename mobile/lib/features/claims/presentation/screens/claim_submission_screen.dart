...
class _ClaimSubmissionScreenState extends State<ClaimSubmissionScreen> {
  ...
  final _categories = ['Transport', 'Makan', 'Medis', 'Parkir', 'Internet', 'Lainnya'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: AppColors.surface, elevation: 0,
        ...
        title: Text('Ajukan Klaim', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: AppColors.primary, fontWeight: FontWeight.bold)), centerTitle: true),
      body: SafeArea(child: SingleChildScrollView(padding: EdgeInsets.all(24.w), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        _label('KATEGORI'),
        SizedBox(height: 8.h),
        DropdownButtonFormField<String>(value: _category, items: _categories.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(),
          onChanged: (v) { if (v != null) setState(() => _category = v); }, decoration: _deco('Pilih kategori')),
        SizedBox(height: 24.h),

        _label('JUMLAH (Rp)'),
        SizedBox(height: 8.h),
        TextFormField(controller: _amountController, keyboardType: TextInputType.number, decoration: _deco('e.g. 150000')),
        SizedBox(height: 24.h),

        _label('DESKRIPSI'),
        SizedBox(height: 8.h),
        TextFormField(controller: _descController, maxLines: 3, decoration: _deco('Jelaskan detail klaim...')),
        SizedBox(height: 24.h),

        _label('TANGGAL PENGELUARAN'),
        SizedBox(height: 8.h),
        InkWell(onTap: () {}, borderRadius: BorderRadius.circular(12.r), child: Container(
          ...
          child: Row(children: [Icon(Icons.calendar_today, color: AppColors.onSurfaceVariant, size: 20.w), SizedBox(width: 12.w),
            Text('Pilih tanggal', style: TextStyle(color: AppColors.onSurfaceVariant.withOpacity(0.5), fontSize: 14.sp))]),
        )),
        SizedBox(height: 24.h),
        ...
        InkWell(onTap: () {}, borderRadius: BorderRadius.circular(12.r), child: Container(
          ...
          decoration: BoxDecoration(color: AppColors.surfaceContainerLow, borderRadius: BorderRadius.circular(12.r), border: Border.all(color: AppColors.outlineVariant.withOpacity(0.5))),
          child: Column(children: [Icon(Icons.camera_alt_outlined, color: AppColors.onSurfaceVariant, size: 32.w), SizedBox(height: 8.h),
            Text('Foto atau upload struk', style: TextStyle(color: AppColors.onSurfaceVariant, fontSize: 12.sp))]),
        )),
        SizedBox(height: 32.h),

        SizedBox(height: 52.h, child: ElevatedButton(
          onPressed: () { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Klaim berhasil dikirim!'))); context.pop(); },
          ...
          child: Text('Kirim Klaim', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16.sp)),
        )),
      ]))),
    );
  }
